...
class MemosClient:
    ...
    def list_memos(
        ...
        params = {"page": page, "limit": limit, "orderBy": order_by, "order": order}
        if tag:
            params["tag"] = tag
        if visibility:
    ...
    def create_memo(self, memo_data: CreateMemoRequest) -> Memo:
        response = self._request("POST", "/memos", json=memo_data.model_dump())
    ...
    def update_memo(self, memo_id: int, memo_data: UpdateMemoRequest) -> Memo:
        response = self._request(
            "PATCH", f"/memos/{memo_id}", json=memo_data.model_dump(exclude_none=True)
        )
        return Memo(**response.json())
    ...
    # Search
    def search_memos(
        self, query: str, limit: Optional[int] = None, tag: Optional[str] = None
    ) -> SearchResponse:
        params = {"q": query}
        if limit:
            params["limit"] = limit
    ...
    def create_webhook(self, webhook_data: CreateWebhookRequest) -> Webhook:
        response = self._request("POST", "/webhooks", json=webhook_data.model_dump())
